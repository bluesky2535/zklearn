...
//		String root = "cn=admin,dc=ts,dc=com";  // 用户
		String root = "cn=liu da,cn=TS,dc=ts,dc=com";  // 用户
		String pwd ="123456";  // pwd
		LdapContext ctx=ldapConnect(url,root,pwd);//cdh ldap认证
		List<LdapUser> cdhlm=readLdap(ctx,basedn);//获取cdh ldap中用户信息
		 //通过时间戳初始化一个uid
		 long init_uid= getNowTimeStamp();
		 //循环集团ldap用户
		 for(int i=0;i<cdhlm.size();i++){
			 LdapUser jtu=cdhlm.get(i);
	...
	/**
	  ...
	  public static boolean addGoups(LdapUser lu,LdapContext ctx) {
	      ...
	      try {
	    	String cn="cn="+lu.getCn()+",dc=ts,dc=com";
...
//	    	String dn="uid="+lu.getCn()+" ,cn=TS,dc=ts,dc=com";
	    	String dn="cn="+lu.getCn() +",cn=TS,dc=ts,dc=com";
...
//				String[] attrPersonArray = {"cn","email"};
	            SearchControls searchControls = new SearchControls();//搜索控件

//			     OBJECT_SCOPE 搜索命名对象
//				 ONELEVEL_SCOPE 搜索命名上下文的一个级别
//				 SUBTREE_SCOPE 搜索以命名对象为根的整个子树

	            searchControls.setSearchScope(0);//搜索范围
